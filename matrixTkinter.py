...
# METTERE QUI IL POP UP DEELLE DIMENSIONI IN INPUT DELLE MATRICI
...
def get_pattern_270():
    ...
    final_matrix = [[new_new_matrix[j][i] for j in range(rows_patter)] for i in
                    range(len(new_new_matrix[0]) - 1, -1, -1)]
    print(final_matrix)
    return final_matrix
...
def get_result():
    ...
    open_popup_find(n_patter_0, n_patter_90, n_patter_180, n_patter_270, matrix, matrix_90, matrix_180, matrix_270, 0, 90, 180, 270)
...
def set_all_zero():
    ...
    for i in range(rows_patter):
        matrix.append([])
        for j in range(cols_pattern):
            matrix[i].append(entries[i][j].deselect())
...
def set_all_zero_prof():
    ...
    for i in range(rows):
        matrix.append([])
        for j in range(cols):
            matrix[i].append(entries_prof[i][j].deselect())
...
for i in range(rows_patter):
    ...
    for j in range(cols_pattern):
        ...
        entries[i].append(Checkbutton(window, variable=text_var[i][j], width=3))  # here we set the dimension on squares
...
for i in range(rows):
    ...
    for j in range(cols):
        # append your StringVar and Entry
        text_var_prof[i].append(IntVar())
        entries_prof[i].append(
            Checkbutton(window, variable=text_var_prof[i][j], width=3))  # here we set the dimension on squares
